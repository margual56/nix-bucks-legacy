...
mod tests {
    ...
    #[allow(unused)]
    use crate::{utils::times_until, Recurrence};
    ...
    fn test_times_until_months() {
        assert_eq!(times_until(Recurrence::Month(1, 1), *START, *TARGET), 7);
        assert_eq!(times_until(Recurrence::Month(1, 2), *START, *TARGET), 3);
        assert_eq!(times_until(Recurrence::Month(1, 3), *START, *TARGET), 2);
    }
    ...
    fn test_times_until_years() {
        ...
        assert_eq!(times_until(Recurrence::Year(1, 1, 1), *START, target), 10);
        assert_eq!(times_until(Recurrence::Year(1, 1, 2), *START, target), 5);
        assert_eq!(times_until(Recurrence::Year(1, 1, 3), *START, target), 3);
    }
}
